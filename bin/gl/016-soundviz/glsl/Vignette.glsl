...
void main(void) {
	...
	float d = distance(vTextureCoord, vec2(0.5, 0.5)) * uBeat;

	vec2 vt = vTextureCoord;

	float limit = 0.4 + vt.y * 0.2;

	// vt.y += (vt.x > limit) ? -0.04 * uBeat : 0.04 * uBeat;
	vt.x += (smoothstep(0.25, 0.75, fract(vt.y * 80.0)) * 0.08 - 0.04) * (uBeat);
    ...
    vec3 c = vec3(cr, cg, cb) + whiteNoise(vTextureCoord, uBeat * cr * 1.2);
