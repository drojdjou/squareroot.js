 ...
 *
 *  @see createTexture in {@link SQR.SquarerootGL}
 *
 *  @param gl The webGL context. Since this object is not available you cannot create a texutre directly. Use createTexture in {@link SQR.SquarerootGL} instead.
 ...
 *  @param params.onLoad {function} a callback to call when all the image is loaded 
 *  @param params.mipmap {boolean} create a mipmap. Defaults to true.
 *  @param params.flip {boolean} flip the texture on load. Defaults to true.
 *  @param params.magFilter {number} the filter to apply when magnifying the image (@todo implement constants for this)
 *  @param params.minFilter {number} the filter to apply when minifying the image (@todo implement constants for this)
 *  @param params.wrapMode {number} the wrap mode for the texture (@todo implement constants for this)
 *  @param params.autoPlay {boolean} if texture is video, should it start playing automatically? Defaults to false.
 *  @param params.cors {string} cross origin value to set on the image (@todo more details please)
 *  @param params.onEnded {function} a callback to invoke when the video ends
 *  @param params.loop {function} if true video will be looped. Defaults to false.
 *  @param params.image {boolean} if true states that the texture is an image. 
 *      The type is figure out by extension, but if texture is pulled from ex. a server side script than, 
 *      it might be necessary to spcify the type with this variable.
 *  @param params.image {boolean} if true states that the texture is a video. 
 *      The type is figure out by extension, but if texture is pulled from ex. a server side script than, 
 *      it might be necessary to spcify the type with this variable.
 */
SQR.Texture = function(gl, source, params) {
    var that = this;
    ...
    /**
     *  @returns true is the texture size id power of 2 (128, 256, 512, etc)
     */
    ...
    /**
     *  Updates the texture in memory if it is a video. 
     *  @force if true updates the texture anyay, even if it is not a video
     */
